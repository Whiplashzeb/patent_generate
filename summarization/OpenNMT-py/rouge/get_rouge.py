...
class Rouge:
    ...
    def __init__(self, metrics=None, max_n=None, limit_length=True, length_limit=665, length_limit_type='bytes', apply_avg=True, apply_best=False, stemming=True, alpha=0.5, weight_factor=1.0, ensure_compatibility=False):
        """
...
for aggregator in ['Avg']:
    ...
    with open(reference_file) as fp:
        line = fp.readline()
        while len(line) > 0:
            all_references.append(line.strip())
            line = fp.readline()

    with open(hypothesis_file) as fp:
        line = fp.readline()
        while len(line) > 0:
            all_hypothesis.append(line.strip())
            line = fp.readline()
